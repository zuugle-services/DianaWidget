  ...
  renderConnectionDetails(connections, type) {
    ...
    return connections.map(conn => {
      ...
      if (type === 'to' && this.state.activityTimes.start) {
        const activityStart = DateTime.fromFormat(this.state.activityTimes.start, 'HH:mm');
        const connectionEnd = DateTime.fromISO(conn.connection_end_timestamp);
        const waitDuration = activityStart.diff(connectionEnd, ['minutes', 'seconds']);
      ...
      if (type === 'from' && this.state.activityTimes.end) {
        const activityEnd = DateTime.fromFormat(this.state.activityTimes.end, 'HH:mm');
        const connectionStart = DateTime.fromISO(conn.connection_start_timestamp);
        const waitDuration = connectionStart.diff(activityEnd, ['minutes', 'seconds']);
